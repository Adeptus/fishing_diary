class Json::CommentsController < ApplicationController
  ...
  def index
    ...
    @comments = @commentable.root_comments.order('created_at asc').page(params[:page])
    render json: @comments.map(&:attributes_to_json)
  end

  def children_comments
    @comment = Comment.find(params[:id])
    @comments = @comment.children.except(:order).order('created_at asc').page(params[:page]).per(10)
